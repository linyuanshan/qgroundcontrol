...
`clang-tidy` must use the `compile_commands.json` generated by the active CMake build. Pass the
directory containing that file with `--build-dir`. Windows does not create the repository-root
symlink used on other platforms, so an out-of-source build must be selected explicitly:

```powershell
python tools/analyze.py custom/src/Marine/Geometry --tool clang-tidy --build-dir build/P0-01-marine-debug
```

On Windows, the analyzer disables MSVC precompiled-header consumption for clang-tidy while
retaining the compilation database's forced include. It also suppresses Clang's diagnostic for
MSVC-only command-line arguments. This avoids trying to load an MSVC-generated `.pch` file as a
Clang PCH without hiding source diagnostics.
